...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(
      `https://utilization-model-include-hungry.trycloudflare.com/summarize_abstractive/${arsip.id}`,
    )
  ...
  try {
    const resRekomendasi = await axios.get(
      `https://utilization-model-include-hungry.trycloudflare.com/recommendation/${arsip.id}`,
    )
