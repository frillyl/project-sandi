...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(
      `https://combining-crops-studios-emphasis.trycloudflare.com/summarize_abstractive/${arsip.id}`,
    )
  ...
  try {
    const resRekomendasi = await axios.get(
      `https://combining-crops-studios-emphasis.trycloudflare.com/recommendation/${arsip.id}`,
    )
