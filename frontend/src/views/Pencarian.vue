...
const getFileUrl = (arsip) => {
  return `https://ls-nw-ireland-sells.trycloudflare.com/storage/${arsip.file_path}`
}
...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(
      `https://ls-nw-ireland-sells.trycloudflare.com/summarize_abstractive/${arsip.id}`,
    )
  ...
  try {
    const resRekomendasi = await axios.get(
      `https://ls-nw-ireland-sells.trycloudflare.com/recommendation/${arsip.id}`,
    )
