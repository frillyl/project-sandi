...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://suspected-outputs-telephone-mating.trycloudflare.com/summarize/${arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://suspected-outputs-telephone-mating.trycloudflare.com/recommendation/${arsip.id}`)
