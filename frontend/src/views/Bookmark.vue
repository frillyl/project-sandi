...
const toggleBookmark = async (arsip) => {
  ...
  if (isBookmarked(arsip.id)) {
    try {
      await axios.delete(
        `https://smaller-owned-sides-tourist.trycloudflare.com/api/bookmarks/${arsip.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://combining-crops-studios-emphasis.trycloudflare.com/summarize/${arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://combining-crops-studios-emphasis.trycloudflare.com/recommendation/${arsip.id}`)
