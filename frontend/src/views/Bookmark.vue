...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://parker-noise-performed-lips.trycloudflare.com/summarize/${arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://parker-noise-performed-lips.trycloudflare.com/recommendation/${arsip.id}`)
