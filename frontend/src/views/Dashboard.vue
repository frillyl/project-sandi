  ...
  if (token) {
    try {
      const response = await axios.get(
        'https://suburban-designated-authorized-conditional.trycloudflare.com/api/user',
        {
          headers: {
...
const getFileUrl = (arsip) => {
  return `https://suburban-designated-authorized-conditional.trycloudflare.com/storage/${arsip.arsip.file_path}`
}

const downloadFile = async (arsip) => {
  try {
    const response = await axios.get(
      `https://suburban-designated-authorized-conditional.trycloudflare.com/api/arsip/download/${arsip.arsip.id}`,
      {
        responseType: 'blob',
      },
    )
