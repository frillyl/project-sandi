  ...
  if (token) {
    try {
      const response = await axios.get(
        'https://mississippi-friends-place-homepage.trycloudflare.com/api/user',
        {
          headers: {
...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://parker-noise-performed-lips.trycloudflare.com/summarize/${arsip.arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://parker-noise-performed-lips.trycloudflare.com/recommendation/${arsip.arsip.id}`)
...
const getFileUrl = (arsip) => {
  return `https://mississippi-friends-place-homepage.trycloudflare.com/storage/${arsip.arsip.file_path}`
}

const downloadFile = async (arsip) => {
  try {
    const response = await axios.get(
      `https://mississippi-friends-place-homepage.trycloudflare.com/api/arsip/download/${arsip.arsip.id}`,
      {
        responseType: 'blob',
      },
    )
