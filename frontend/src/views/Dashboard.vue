  ...
  if (token) {
    try {
      const response = await axios.get(
        'https://cyber-polo-urge-theories.trycloudflare.com/api/user',
        {
          headers: {
...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://summit-boating-mins-salon.trycloudflare.com/summarize/${arsip.arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://summit-boating-mins-salon.trycloudflare.com/recommendation/${arsip.arsip.id}`)
...
const getFileUrl = (arsip) => {
  return `https://cyber-polo-urge-theories.trycloudflare.com/storage/${arsip.arsip.file_path}`
}

const downloadFile = async (arsip) => {
  try {
    const response = await axios.get(
      `https://cyber-polo-urge-theories.trycloudflare.com/api/arsip/download/${arsip.arsip.id}`,
      {
        responseType: 'blob',
      },
    )
