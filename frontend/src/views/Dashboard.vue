  ...
  if (token) {
    try {
      const response = await axios.get(
        'https://pupils-immigrants-gmt-associations.trycloudflare.com/api/user',
        {
          headers: {
...
const openModal = async (arsip) => {
  ...
  try {
    const resSummary = await axios.get(`https://guest-enters-rolling-females.trycloudflare.com/summarize/${arsip.arsip.id}`)
  ...
  try {
    const resRekomendasi = await axios.get(`https://guest-enters-rolling-females.trycloudflare.com/recommendation/${arsip.arsip.id}`)
...
const getFileUrl = (arsip) => {
  return `https://pupils-immigrants-gmt-associations.trycloudflare.com/storage/${arsip.arsip.file_path}`
}

const downloadFile = async (arsip) => {
  try {
    const response = await axios.get(
      `https://pupils-immigrants-gmt-associations.trycloudflare.com/api/arsip/download/${arsip.arsip.id}`,
      {
        responseType: 'blob',
      },
    )
