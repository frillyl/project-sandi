...
const fetchKlasifikasi = async () => {
  try {
    const response = await axios.get(
      'https://used-envelope-tvs-improvement.trycloudflare.com/api/klasifikasi',
    )
...
const submitForm = async () => {
  ...
  try {
    if (isEditMode.value) {
      ...
      await axios.post(
        `https://used-envelope-tvs-improvement.trycloudflare.com/api/arsip/${selectedId.value}`,
        formData,
        {
          headers: {
...
const deleteArsip = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`https://used-envelope-tvs-improvement.trycloudflare.com/api/arsip/${id}`)
      await fetchArsip()
      Swal.fire({
        icon: 'success',
        title: 'Berhasil',
